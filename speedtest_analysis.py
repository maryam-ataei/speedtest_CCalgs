...
output_path = "/home/maryam/SEARCH/speedtest_CCalgs/results/speedtest_2"
input_path = "/home/maryam/SEARCH/speedtest_CCalgs/2/speedtest_search_over_homecable_2.txt"

server_log_path = "/home/maryam/SEARCH/speedtest_CCalgs/2/server_speedtest_2/data/log_search"
server_pcap_path = "/home/maryam/SEARCH/speedtest_CCalgs/2/server_speedtest_2/data/pcap_server"

client_pcap_path = "/home/maryam/SEARCH/speedtest_CCalgs/pcap_dumps"
...
if not os.path.exists(client_pcap_path):
    ...
    if not client_pcap_files:
        ...
        exit()
    num_client_pcap_files = len(client_pcap_files) - 1  # Get the last file

    for k in range((num_client_pcap_files + 1)// 2):
        client_pcap_file_postspeed_path = os.path.join(client_pcap_path, f"upload_post_run_{k+1}.pcap")
        if not os.path.exists(client_pcap_file_postspeed_path):
            print(f"File {client_pcap_file_postspeed_path} does not exist")
            continue
        
        client_pcap_file_prespeed_path = os.path.join(client_pcap_path, f"upload_pre_run_{k+1}.pcap")
        ...
        for client_csv_file_path in client_csv_file_path_list:
            ...
            if client_csv_file_path.endswith(f"post_run_{k+1}.csv"):
                ...
                upload_duration_pre_list.append(upload_bits_sent)
...
if avg_delivery_rates:
    ...
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.xlim(left=60)  # Set x-axis to start from 0
    plt.tight_layout()
...
if upload_bits_sent_pre_list and total_bits_acked_at_exit_list:
    plt.figure()
    plt.plot(range(1, num_log_files + 2), upload_bits_sent_pre_list, marker='o', label='Upload Bits Sent Pre')
    plt.plot(range(1, num_log_files + 2), total_bits_acked_at_exit_list, marker='o', color='m', label='Total Bits Acked at Exit')
    plt.xlabel("Run", fontsize=14)
    plt.ylabel("Bits", fontsize=14)
    plt.title("Upload Bits Sent Pre and Total Bits Acked at Exit")
    plt.legend()
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "upload_bits_sent_pre_and_total_bits_acked_at_exit.png"))
    plt.close()

# plot upload bits sent post and search bit acked at exit
if upload_bits_sent_post_list and total_bits_acked_at_exit_list:
    plt.figure()
    plt.plot(range(1, num_log_files + 2), upload_bits_sent_post_list, marker='o', label='Upload Bits Sent Post')
    plt.plot(range(1, num_log_files + 2), total_bits_acked_at_exit_list, marker='o', color='m', label='Total Bits Acked at Exit')
    plt.xlabel("Run", fontsize=14)
    plt.ylabel("Bits", fontsize=14)
    plt.title("Upload Bits Sent Post and Total Bits Acked at Exit")
    plt.legend()
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "upload_bits_sent_post_and_total_bits_acked_at_exit.png"))
    plt.close()
