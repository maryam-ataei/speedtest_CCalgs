...
output_path = "/home/maryam/SEARCH/speedtest_CCalgs/14_speedtest_server_linux_desktop_starbucks_4g/paper_result_new3"
if not os.path.exists(output_path):
    os.makedirs(output_path)

# THROUGHPUT_WINDOW_BINS = 0.04 
client_ip = "172.20.10.4" #FIOS_WIRELESS:"192.168.4.174" #FIOS_CABLE:"192.168.1.10"  #4G LTE: '172.20.10.4'    #SPECTRUM: "192.168.1.100"  #GEO: "192.168.1.107"
...
ERROR_GRAPH_ALLS = False
error_path_files = "/home/maryam/SEARCH/speedtest_CCalgs/error_data/IPCCC_2025"
#################################### Functions ####################################
...
######################## Make Error graphs of all tools over all networks in one graph
if ERROR_GRAPH_ALLS:
    # iterate over folders and in each folder there is a csv file that have the avg of error, avg of duration, avg of byte sent 

    folders = []
    folders = [f for f in os.listdir(error_path_files) if os.path.isdir(os.path.join(error_path_files, f))]
    if not folders:
        print(f"No folders found in {error_path_files}")
        exit()

    # create a dictionary to store the data
    data = {}
    for folder in folders:
        folder_path = os.path.join(error_path_files, folder)
        ...
        if 'Avg Error' not in df.columns or 'Avg Bytes Sent (MB)' not in df.columns or 'Avg Duration (seconds)' not in df.columns or 'Tool' not in df.columns:
            print(f"Required columns not found in {csv_file_path}")
            continue

        # store the data in the dictionary for each folder 
        data[folder] = {
            'Tool': df['Tool'].tolist(),
            'Avg Error': df['Avg Error'].tolist(),
            'Avg Bytes Sent (MB)': df['Avg Bytes Sent (MB)'].tolist(),
            'Avg Duration (seconds)': df['Avg Duration (seconds)'].tolist()
        }
    
    # Marker style for each network
    network_markers = {
        '4G LTE': 'o',
        'Cable': 's',
        'GEO': '^'
    ...
    # Create the plot Error vs Duration
    plt.figure(figsize=(10, 6))

    # Loop through each network
    for network, values in data.items():
        tools = values['Tool']
        errors = values['Avg Error']
        durations = values['Avg Duration (seconds)']
        
        # Loop through each tool in the current network
        for tool, error, duration in zip(tools, errors, durations):
            # Plot each network-tool combination with respective marker and color
            plt.scatter(duration, error,
                        marker=network_markers.get(network, 'x'),
                        color=tool_colors.get(tool, 'gray'),
                        s=100,
                        label=tool if network == '4G LTE' and tool == 'ookla' else "")

    # Axis labels and title
    plt.xlabel('Duration (seconds)', fontsize=24)
    plt.ylabel('Error', fontsize=24)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)

    ###### Adding grid and legend 
    # Legend entries for Tools (marker shapes)
    tool_handles = [
        Line2D([0], [0], marker='o', color='black', linestyle='', markersize=10, label='4G LTE'),
        Line2D([0], [0], marker='s', color='black', linestyle='None', markersize=10, label='Cable'),
        Line2D([0], [0], marker='^', color='black', linestyle='None', markersize=10, label='GEO')
    ]

    # Legend entries for Method (colors)
    method_handles = [
        Patch(facecolor=tool_colors['Ookla'], label='Speedtest'),
        Patch(facecolor=tool_colors['NDT7'], label='NDT7'),
        Patch(facecolor=tool_colors['LibreSpeed'], label='Libre'),
        Patch(facecolor=tool_colors['Search'], label='SEARCH')
    ]

    # Combine into a single list with a blank separator
    combined_handles = (
        tool_handles +
    ...
    plt.legend(handles=combined_handles, loc='upper right', frameon=True, fontsize=16, title_fontsize=18)

    # #### Adding oval around same Technology
    # # Group all (x, y) points by tool
    # tool_points = {}
    # for network, values in data.items():
    #     tools = values['Tool']
    #     errors = values['Avg Error']
    ...
    #     for tool, error, duration in zip(tools, errors, durations):
    #         if tool not in tool_points:
    #             tool_points[tool] = []
    #         tool_points[tool].append((duration, error))  # (x, y)

    # # Draw PCA-based ellipse for each method (refined to match spread tightly)
    # FORCE_WIDTH_NDT7 = 0.1  # force minimum width for NDT7
    # FORCE_HEIGHT_NDT7 = 1.1  # optional: also slightly boost height if needed

    # for tool, points in tool_points.items():
    #     if tool == 'Ookla':
    #         continue  # Skip ellipse for Ookla
    #     if len(points) < 2:
    #         continue
    ...
    #     width = 2 * 1.37 * stds[0]
    #     height = 2 * 1.37 * stds[1]

    #     # Only boost NDT7
    #     if tool == 'NDT7':
    #         width = max(width, FORCE_WIDTH_NDT7)
    #         height = max(height, FORCE_HEIGHT_NDT7)
    ...
    #         height=height,
    #         angle=angle,
    #         edgecolor=tool_colors.get(tool, 'gray'),
    #         facecolor=tool_colors.get(tool, 'gray'),
    #         alpha=0.1,
    #         linewidth=2
    #     )
    #     plt.gca().add_patch(ellipse)

    # Adjust layout if overlapping
    plt.tight_layout()

    plt.xlim(-0.9, 55)
    plt.ylim(-0.1, 1.5)
    # Save the figure to the specified path
    plt.savefig(os.path.join(error_path_files, 'avg_error_vs_avg_duration_all_tools.png'))

    # Close the plot to avoid display in non-interactive environments
    plt.close()


    # Create the plot for Error vs Byte sent
    plt.figure(figsize=(10, 6.2))
    # Loop through each network
    for network, values in data.items():
        tools = values['Tool']
        errors = values['Avg Error']
        byte_sent = values['Avg Bytes Sent (MB)']
        
        # Loop through each tool in the current network
        for tool, error, byte_sent in zip(tools, errors, byte_sent):
            # Plot each network-tool combination with respective marker and color
            plt.scatter(byte_sent, error,
                        marker=network_markers.get(network, 'x'),
                        color=tool_colors.get(tool, 'gray'),
                        s=100,
                        label=tool if network == '4G LTE' and tool == 'ookla' else "")
    ...
    # Axis labels and title
    plt.xlabel('Byte Sent (MB)', fontsize=24)
    plt.ylabel('Error', fontsize=24)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)
        ###### Adding grid and legend 
    ...
    # Combine into a single list with a blank separator
    combined_handles = (
        # [Line2D([0], [0], linestyle='None', label='Tools:')] +
        tool_handles +
        # [Line2D([0], [0], linestyle='None', label='')] +  # Blank line
        [Line2D([0], [0], linestyle='None')] +
        method_handles
    )
    ...
    #     for tool, error, duration in zip(tools, errors, byte_sent):
    #         if tool not in tool_points:
    #             tool_points[tool] = []
    #         tool_points[tool].append((duration, error))  # (x, y)
    ...
    #     # Use standard deviation (explained_variance_) for ellipse radii
    #     width, height = 1.9 * 1.3 * np.sqrt(pca.explained_variance_)  # 1.37 std devs
    #     angle = np.degrees(np.arctan2(pca.components_[0, 1], pca.components_[0, 0]))
    ...
    plt.ylim(-0.1, 1.5)
    # Save the figure to the specified path
    plt.savefig(os.path.join(error_path_files, 'avg_error_vs_avg_bytes_all_tools.png'))
    ...
    if ookla_data and ndt7_data and libre_data and delivery_rate_at_exit_on_acked_bytes_list:
        ...
        plt.plot(cdf_diff_ookla, cdf_values_diff_ookla, marker='o', color='blue', label='(Speedtest - DR at Exit)')
        ...
        # plot vertical dashed line on zero
        plt.axvline(x=0, color='gray', linestyle='--')
        plt.plot(cdf_norm_diff_ookla, cdf_values_norm_diff_ookla, marker='o', color='blue', label='norm diff(Speedtest, DR at Exit)')
    ...
    if median_throughputs and ookla_data and ndt7_data and libre_data:
        ...
        plt.plot(cdf_norm_diff_ookla, cdf_values_norm_diff_ookla, marker='*', color='blue', label='Speedtest')
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(cdf_abs_norm_diff_search, cdf_values_abs_norm_diff_search, marker='o',  markersize=8, color='purple', label='SEARCH')
    plt.plot(cdf_abs_norm_diff_ookla, cdf_values_abs_norm_diff_ookla, marker='*',  markersize=8, color='blue', label='Speedtest')
    plt.plot(cdf_abs_norm_diff_ndt7, cdf_values_abs_norm_diff_ndt7, marker='X',  markersize=8, color='darkorange', label='NDT7')   
    plt.plot(cdf_abs_norm_diff_libre, cdf_values_abs_norm_diff_libre, marker='^', markersize=8, color='green', label='Libre')
    plt.xlabel("Normalized Absolute Difference", fontsize=19)
    plt.ylabel("Cumulative Distribution", fontsize=19)
    ...
    # make them as percentage
    absolute_norm_diff_ookla_percntage = [d * 100 for d in absolute_norm_diff_ookla]
    absolute_norm_diff_ndt7_percntage = [d * 100 for d in absolute_norm_diff_ndt7]
    absolute_norm_diff_libre_percntage = [d * 100 for d in absolute_norm_diff_libre]
    absolute_norm_diff_median_throughput_dr_exit_bytes_percntage = [d * 100 for d in absolute_norm_diff_median_throughput_dr_exit_bytes]

    # cdf 
    cdf_Abs_norm_diff_ookla_perc, cdf_values_abs_norm_diff_ookla_perc = calculate_cdf(absolute_norm_diff_ookla_percntage)
    cdf_Abs_norm_diff_ndt7_perc, cdf_values_abs_norm_diff_ndt7_perc  = calculate_cdf(absolute_norm_diff_ndt7_percntage)
    cdf_Abs_norm_diff_libre_perc, cdf_values_abs_norm_diff_libre_perc = calculate_cdf(absolute_norm_diff_libre_percntage)
    cdf_Abs_norm_diff_search_perc, cdf_values_abs_norm_diff_search_perc = calculate_cdf(absolute_norm_diff_median_throughput_dr_exit_bytes_percntage)  

    # plot
    plt.figure(figsize=(9, 6))
    # plot vertical dashed line on zero
    plt.axvline(x=0, color='gray', linestyle='--')
    plt.plot(cdf_Abs_norm_diff_search_perc, cdf_values_abs_norm_diff_search_perc, marker='o', markersize=8, color='purple', label='SEARCH')
    plt.plot(cdf_Abs_norm_diff_ookla_perc, cdf_values_abs_norm_diff_ookla_perc, marker='*',  markersize=9,  color='blue', label='Speedtest')
    plt.plot(cdf_Abs_norm_diff_ndt7_perc, cdf_values_abs_norm_diff_ndt7_perc, marker='X',  markersize=8,  color='darkorange', label='NDT7')   
    plt.plot(cdf_Abs_norm_diff_libre_perc, cdf_values_abs_norm_diff_libre_perc, marker='^',  markersize=8,  color='green', label='Libre')
    plt.xlabel("Normalized Absolute Difference (%)", fontsize=28)
    plt.ylabel("Cumulative Distribution", fontsize=28)
    # plt.title("CDF of Absolute Normalized Difference between Median Throughput and Speedtest Tools")
    plt.legend(fontsize=20)
    plt.xticks(fontsize=24)
    plt.yticks(fontsize=24)
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "cdf_abs_norm_diff_median_throughput_and_speedtest_tools_percentage.png"))
    plt.close()  
    ...
    if ookla_metrics and ndt7_metrics and libre_metrics and all_ss_exit_time_list:
        ...
        plt.figure(figsize=(9, 6))
        plt.plot(cdf_ookla_dwnd_duration, cdf_values_ookla_dwnd_duration, marker='*', markersize=9,  color='blue', label='Speedtest')
        plt.plot(cdf_all_ss_exit_time, cdf_values_all_ss_exit_time, marker='o', markersize=8, color='purple', label='SEARCH')
        plt.plot(cdf_ndt7_dwnd_duration, cdf_values_ndt7_dwnd_duration, marker='X', markersize=8,  color='darkorange', label='NDT7')
        plt.plot(cdf_libre_dwnd_duration, cdf_values_libre_dwnd_duration, marker='^', markersize=8,  color='green', label='Libre')

        plt.xlabel("Duration (seconds)", fontsize=28)
        plt.ylabel("Cumulative Distribution", fontsize=28)
        # plt.title("CDF of Download Durations and All SS Exit Time")   
        plt.legend(fontsize=20)
        plt.xticks(fontsize=24)
        plt.yticks(fontsize=24)
        plt.xlim([-0.5, 43])  # Set x-axis to start from
        plt.tight_layout()
    ...
    plt.figure(figsize=(9, 6))
    plt.plot(cdf_ookla_byte_sent, cdf_values_ookla_byte_sent, marker='*', markersize=9, color='blue', label='Speedtest')
    plt.plot(cdf_all_ss_exit_byte_acked, cdf_values_all_ss_exit_byte_acked, marker='o', markersize=8, color='purple', label='SEARCH')
    plt.plot(cdf_ndt7_byte_sent, cdf_values_ndt7_byte_sent, marker='X', markersize=8, color='orange', label='NDT7')
    plt.plot(cdf_libre_byte_sent, cdf_values_libre_byte_sent, marker='^', markersize=8, color='green', label='Libre')
    plt.xlabel("Bytes Sent (MB)", fontsize=28) 
    plt.ylabel("Cumulative Distribution", fontsize=28)
    ...
    plt.xticks(fontsize=24)
    plt.yticks(fontsize=24)
    plt.xlim([-5, 240])  # Set x-axis to start from
    plt.tight_layout()
    ...
    if ookla_data and ndt7_data and libre_data and median_throughputs:
        ookla_avg_duration = np.average([v["download_duration"] for v in ookla_metrics.values() if v["download_duration"] is not None])
        ndt7_avg_duration = np.average([v["download_duration"] for v in ndt7_metrics.values() if v["download_duration"] is not None])
        libre_avg_duration = np.average([v["download_duration"] for v in libre_metrics.values() if v["download_duration"] is not None])
        #also for SEARCH (all ss exit time)
        search_avg_duration = np.average(all_ss_exit_time_list) if all_ss_exit_time_list else None

        # for error: avg norm diff between median throughput and download speed of each tool
        error_ookla = np.average([abs(median - speed)/median for median, speed in zip(median_throughputs, [d["Download"] for d in ookla_data]) if speed is not None])
        error_ndt7 = np.average([abs(median - speed)/median for median, speed in zip(median_throughputs, [d["Download"] for d in ndt7_data]) if speed is not None])   
        error_libre = np.average([abs(median - speed)/median for median, speed in zip(median_throughputs, [d["Download"] for d in libre_data]) if speed is not None])
        error_search = np.average([abs(median - speed)/median for median, speed in zip(median_throughputs, delivery_rate_at_exit_on_acked_bytes_list)]) 
        # create a scatter plot
        plt.figure(figsize=(8, 5))
        plt.scatter(ookla_avg_duration, error_ookla, color='blue', marker='*', s=100, label='Speedtest')
        plt.scatter(ndt7_avg_duration, error_ndt7, color='orange', marker='s', s=100, label='NDT7')
        plt.scatter(libre_avg_duration, error_libre, color='green', marker='^', s=100, label='Libre')
        ...
        plt.ylabel("Avg Norm Difference", fontsize=19)
        ...
        plt.figure(figsize=(8, 5))
        plt.scatter(ookla_avg_byte_sent, error_ookla, color='blue', marker='*', s=100, label='Speedtest')
        plt.scatter(ndt7_avg_byte_sent, error_ndt7, color='orange', marker='s', s=100, label='NDT7')
        plt.scatter(libre_avg_byte_sent, error_libre, color='green', marker='^', s=100, label='Libre')
        ...
        tools_data = {
            ...
            "Avg Error": [error_ookla, error_ndt7, error_libre, error_search]
        }
    ...
    if median_throughputs_diff_windows and delivery_rate_at_exit_on_acked_bytes_list:
        #calc cdf for delivery_rate_at_exit_on_acked_bytes_list
        delivery_rate_at_exit_on_acked_bytes_list = [d for d in delivery_rate_at_exit_on_acked_bytes_list if d is not None]
        cdf_delivery_rate_at_exit_on_acked_bytes, cdf_values_delivery_rate_at_exit_on_acked_bytes = calculate_cdf(delivery_rate_at_exit_on_acked_bytes_list)    
        ...
        plt.plot(cdf_delivery_rate_at_exit_on_acked_bytes, cdf_values_delivery_rate_at_exit_on_acked_bytes, marker='X', color='m', label='Delivery Rate at Exit')
